...
export default function CourseDetail() {
  ...
  return (
    <>
      <SEO title={item.title} description={item.shortDescription} path={`/courses/${item.slug}`} />
      <section className="bg-primary text-white">
        <div className="mx-auto max-w-5xl px-4 py-14 lg:px-8">
          <Link to="/courses" className="hover-underline-gold mb-4 inline-block text-sm text-white/70">← Back to Courses</Link>
      ...
      <section className="mx-auto max-w-5xl px-4 py-14 lg:px-8">
        ...
        <div className="mb-10 flex flex-col gap-4">
          {item.modules.map((m, idx) => (
            <div key={m.title} className="rounded-lg border border-neutral-100 bg-white p-5">
