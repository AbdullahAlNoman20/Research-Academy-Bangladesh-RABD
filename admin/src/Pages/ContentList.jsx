...
export default function ContentList({ typeKey }) {
  ...
  return (
    <>
      <SEO
        title={type.label}
        description={`Explore our ${type.label.toLowerCase()} at Research Academy Bangladesh.`}
        path={type.basePath}
      />
      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        ...
        {!loading && !error && items.length === 0 && (
          <EmptyState title={`No ${type.label.toLowerCase()} yet`} />
        )}
        {!loading && !error && items.length > 0 && (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((item) => (
              <ContentCard key={item.id} item={item} basePath={type.basePath} />
            ))}
          </div>
        )}
      </section>
