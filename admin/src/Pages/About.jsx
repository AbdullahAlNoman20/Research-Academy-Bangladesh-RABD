...
const JOURNEY = [
  { year: '2019', text: 'Research Academy Bangladesh founded with a single research methodology course for medical residents.' },
  { year: '2021', text: 'Expanded into biostatistics and scientific writing training, reaching 300+ clinicians.' },
  { year: '2023', text: 'Launched institutional research training programs for hospitals and universities.' },
  { year: '2026', text: 'Serving 1000+ students with courses, workshops, mentorship and publication support nationwide.' }
];
      ...
      <section className="mx-auto max-w-5xl px-4 py-16 lg:px-8">
        ...
        <div className="relative mb-16 flex flex-col gap-8 border-l-2 border-secondary pl-6">
          {JOURNEY.map((j) => (
            <div key={j.year} className="relative">
              <span className="absolute -left-[31px] flex h-6 w-6 items-center justify-center rounded-full bg-secondary text-[10px] font-bold text-primary">{j.year.slice(2)}</span>
              <span className="text-xs font-semibold text-secondary-dark">{j.year}</span>
        ...
        <div className="mb-16 flex flex-col items-start gap-6 rounded-2xl bg-neutral-50 p-8 md:flex-row md:items-center">
          ...
          <div className="flex-1">
            ...
            <p className="mb-3 text-sm font-semibold text-secondary-dark">Founder &amp; Academic Director</p>
        ...
        {team.length === 0 ? <Loader label="Loading team" /> : (
          <Marquee>
            {team.map((m) => <div key={m.id} className="w-48"><TeamMemberCard member={m} /></div>)}
          </Marquee>
        )}
      </section>
    </>
