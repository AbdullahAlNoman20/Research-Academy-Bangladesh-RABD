...
export default function WorkshopDetail() {
  ...
  return (
    <>
      <SEO title={item.title} description={item.shortDescription} path={`/workshops/${item.slug}`} />
      <section className="bg-primary text-white">
        <div className="mx-auto max-w-5xl px-4 py-14 lg:px-8">
          <Link to="/workshops" className="hover-underline-gold mb-4 inline-block text-sm text-white/70">← Back to Workshops</Link>
      ...
      <section className="mx-auto max-w-5xl px-4 py-14 lg:px-8">
        ...
        <div className="mb-10 flex flex-col gap-3">
          {item.timeline.map((t) => (
            <div key={t.time} className="flex flex-col gap-1 rounded-lg border border-neutral-100 bg-white p-4 sm:flex-row sm:items-center sm:gap-4">
              <span className="w-40 shrink-0 text-xs font-semibold text-secondary-dark">{t.time}</span>
        ...
        <p className="mb-10 rounded-lg bg-neutral-50 p-5 text-sm text-neutral-700">{item.outcome}</p>
