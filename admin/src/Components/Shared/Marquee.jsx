...
export default function Marquee({ children, direction = 'left', speed = 'normal', pauseOnHover = true }) {
  ...
  return (
    <div className={`overflow-hidden ${pauseOnHover ? 'pause-on-hover' : ''}`}>
      <div className={`marquee-track ${animClass} ${dirClass}`}>
        ...
        <div className="flex shrink-0 gap-6 pr-6" aria-hidden="true">{children}</div>
      </div>
    </div>
  );
}